...
class Alipay extends Controller
{
  ...
  public function __construct(){
   ...
   // dump($result);
    ...
    'notify_url' =>  ($_SERVER['SERVER_PORT'] == '443' ? 'https://' : 'http://').$_SERVER['HTTP_HOST']."/public/alipay_notify",
    'return_url' =>  ($_SERVER['SERVER_PORT'] == '443' ? 'https://' : 'http://').$_SERVER['HTTP_HOST']."/public/alipay_return",
 ...
 // dump($out_trade);
  ...
  //松币充值业务逻辑
  $result1=Db::name('order')->where("out_trade_no", $out_trade["out_trade_no"])->find();
  $add_cores=100*(int)$result1["total_amount"];
  
  $result2=Db::name('user')->where("username",   $result1["user"])->find();
  
  
  
  
  $result=Db::name('user')->where("username",   $result1["user"])->update(["cores"=> $add_cores+$result2["cores"]]);
  
  if($result==1){
  //跳转到支付结果页面
  echo "<script language=\"javascript\">";
  echo "document.location=\"/#/Result/".$out_trade["out_trade_no"]."\"";
  echo "</script>";
   }
  }
